...
// Check if business is logged in (business_id should be stored in session)
...
// Get the logged-in business ID from session
$business_id = $_SESSION['business_id'];

// Fetch feedback for the logged-in business
$sql = "SELECT * FROM feedback WHERE business_id = ? ORDER BY created_at DESC";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $business_id);
$stmt->execute();
$result = $stmt->get_result();

// Get the selected date range from the form if submitted
$start_date = isset($_GET['start_date']) ? $_GET['start_date'] : '';
$end_date = isset($_GET['end_date']) ? $_GET['end_date'] : '';

// Construct the SQL query with date range filter
$sql = "SELECT * FROM feedback WHERE business_id = ?";
...
if ($start_date && $end_date) {
    $stmt->bind_param("iss", $business_id, $start_date, $end_date);
} else {
    $stmt->bind_param("i", $business_id);
}
