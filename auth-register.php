...
include 'db.php'; // Make sure you have this connection file
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate the contact number (basic format: 10 digits)
    if (!preg_match('/^[0-9]{10}$/', $contact_no)) {
        echo "Error: Invalid contact number. It must be 10 digits.";
    } else {
        // Prepare SQL query to insert data into the 'businesses' table
        $sql = "INSERT INTO businesses (username, business_name, contact_no, email, password) VALUES (?, ?, ?, ?, ?)";

        // Prepare statement
        if ($stmt = $conn->prepare($sql)) {
            // Bind parameters
            $stmt->bind_param("sssss", $username, $business_name, $contact_no, $email, $password);
                                ...
                                <div class="pt-0">
                                    <form method="POST" class="my-4">
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <input class="form-control" name="business_name" type="text" id="business_name" required="" placeholder="Enter your business name">
                                        </div>

                                        <div class="form-group mb-3">
                                            <label for="contact_no" class="form-label">Contact Number</label>
                                            <input class="form-control" name="contact_no" type="text" id="contact_no" required="" placeholder="Enter your contact number">
                                        </div>
                                        <div class="form-group mb-3">
